...
int main (int argc, char const *argv[])
{
  ...
  data_record_file_.open("/home/sun234/record_gp.csv");
  ...
  RProp rprop;
  rprop.init();
  rprop.maximize(gp_v, 500, 0);
  rprop.maximize(gp_r, 500, 0);
  rprop.maximize(gp_beta, 500, 0);
  cout << "rprop_train_end" << endl;

  // total squared error
  int num_ok = 0;
  for(size_t i = 0; i < v.size(); ++i) {
    double x[] = {v.at(i), r.at(i) * 100.0, beta.at(i) * 1000.0, delta.at(i) * 1000.0, Tf.at(i), Tr.at(i)};    // 实际当前状态数组，套入gp，得到f
    double x_v[] = {v.at(i) * 10.0, r.at(i) * 10.0, beta.at(i) * 100.0, delta.at(i) * 100.0, Tf.at(i) * 10.0, Tr.at(i) * 10.0};
  ...
  rprop.maximize(gp_v, 50, 0);
  std::cout << "param_r " << std::endl;
  rprop.maximize(gp_r, 50, 0);
  std::cout << "param_beta " << std::endl;
  rprop.maximize(gp_beta, 50, 0);
  std::cout << "mse_v = " << tss_v/v.size() << std::endl;
  std::cout << "mse_r = " << tss_r/v.size() << std::endl;
  std::cout << "mse_beta = " << tss_beta/v.size() << std::endl;
